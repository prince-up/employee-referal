...
function toAppUser(session: Session): User {
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const hydrate = useCallback((session: Session | null) => {
    ...
    setState({ user: toAppUser(session), token: session.access_token, isAuthenticated: true, isLoading: false });
  }, []);
  useEffect(() => { authService.getSession().then(hydrate).catch(() => setState((s) => ({ ...s, isLoading: false }))); const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => hydrate(session)); return () => subscription.unsubscribe(); }, [hydrate]);
  const login = useCallback(async (email: string, password: string) => { const data = await authService.signIn(email, password); hydrate(data.session); }, [hydrate]);
